...
class SSMCalculator:
    ...
    def eval_all_SSM_metrics(self, light_feature_seq, waveform, file_name):
        ...
        for s in sv:
            ...
            l, S_l, nov_a = self.calc_light_boundaries_and_SSM(light_shifted, ax)
            a, S_a, nov_l = self.calc_audio_boundaries_and_SSM(waveform, ax, s)
            ...
            if 2*max_ks >= len(nov_a) or len(nov_a[max_ks: -max_ks])<2 or len(nov_l[max_ks: -max_ks])<2:
                utils.logger.log("Kernel size bigger than light / audio sample for file " + file_name)
            else:
    ...
    def calc_light_boundaries_and_SSM(self, light_feature_seq, ax):
        ...
        if ax is not None:
            ...
            libfmp.b.plot_matrix(S, Fs=1, cmap=cmap,
                                 title='SSM light', xlabel='Time (seconds)', ylabel='Time (seconds)', colorbar=True, ax = [ax[0][0]], T_coef=T_coef, F_coef=T_coef);
            fig, _ax, line = libfmp.b.plot_signal(trimmed_nov, Fs=3, color='k', title='Nov light', ax = ax[0][1])
            viz_peaks = T_coef[peaks]
            ax[0][1].vlines(x=viz_peaks,  ymin=0, ymax=1, label='Light Peaks', ls='--', colors=['lime'])
            ax[0][1].set_ylim([0.00, 0.40])

        return self.create_time_boundaries(peaks, len(trimmed_nov), self.H_light), S, trimmed_nov

    def calc_audio_boundaries_and_SSM(self, waveform, ax, shiftvalue):
        nov, S = self.calc_audio_novelty(waveform, shiftvalue)
        peaks, trimmed_nov = self.get_trimmed_novelty_peaks(nov, self.L_audio_Kernel)
        if ax is not None:
            T_coef = np.arange(trimmed_nov.shape[0]) / 3
            libfmp.b.plot_matrix(S, Fs=1, cmap=cmap,
                                 title='SSM audio', xlabel='Time (seconds)', ylabel='Time (seconds)', colorbar=True, ax = [ax[1][0]], T_coef=T_coef, F_coef=T_coef);
            fig, _ax, line = libfmp.b.plot_signal(trimmed_nov, Fs=3, color='k', title='Nov audio', ax = ax[1][1])
            viz_peaks = T_coef[peaks]
            ax[1][1].vlines(x=viz_peaks, ymin=0, ymax=1, label='Audio Peaks', ls='--', colors=['lime'])
            ax[1][1].set_ylim([0.00, 0.40])
    ...
    def calc_audio_novelty(self, waveform, shiftvalue):
        x_duration, X, Fs_X, S, I = self.compute_sm_from_waveform(waveform,
    ...
    def compute_sm_from_waveform(self, waveform, L=21, H=1, L_smooth=16, tempo_rel_set=np.array([1]),
                                 shift_set=np.array([0]), strategy='relative', scale=True, thresh=0.15,
        ...
        """
        # Waveform
        Fs = self.model_config.prepro_config['sampling_rate']
        x_duration = waveform.shape[0] / Fs

        # Chroma Feature Sequence and SSM
        C = librosa.feature.chroma_stft(y=waveform, sr=Fs, tuning=0, norm=2, hop_length=self.model_config.prepro_config['hop_length'], n_fft= self.model_config.prepro_config['window_size'])
        Fs_C = Fs / self.model_config.prepro_config['hop_length']
