...
class Decoder(nn.Module):
    ...
    def init_state(self, bsz, device):
        ...
        vec_h = [h0, h1, h2]
        vec_c = [c0, c1, c2]
        out_frame = torch.randn(bsz, self.input_size).to(device)
...
class Model(nn.Module):
    def __init__(self, encoder, decoder, condition_step=10,
                 sliding_window_size=100, lambda_v=0.01, config=None, device=None):
        super().__init__()
        self.config = config
    ...
    def forward(self, src_seq, src_pos, tgt_seq=None, epoch=None, is_training=False):
        if tgt_seq is None or not is_training:
        ...
        enc_outputs, *_ = self.get_encoder_output(src_seq, src_pos)
        ...
        if (is_training):
            ...
            prediction_mask = torch.zeros(seq_len, int(epoch * self.lambda_v))
            mask = torch.cat([prediction_mask, groundtruth_mask], 1).view(-1)[:seq_len]  # for random

            for i in range(seq_len):
                #dec_input = tgt_seq[:, i] if mask[i] == 1 else dec_output.detach()  # old dance rev original code

                if mask[i] == 1 and i-1>=0:
                    dec_input = tgt_seq[:,i-1]
                elif i-1 >=0 and mask[i-1] == 1:
                    dec_input = dec_output.detach()
                else:
                ...
                dec_output = self.sigmoid(dec_output)
                preds.append(dec_output)
        else:
            tgt_seq_len = 0
            if tgt_seq is not None:
                _, tgt_seq_len, _ = tgt_seq.size()

            generated_frames_num = seq_len - tgt_seq_len
    ...
    def get_encoder_output(self, src_seq, src_pos):
        enc_mask = get_subsequent_mask(src_seq, self.sliding_window_size)
        return self.encoder(src_seq, src_pos, mask=enc_mask)

    def predict(self, src_seq, src_pos, tgt_seq=None):
        self.eval()
        with torch.no_grad():
            output = self(src_seq, src_pos, tgt_seq)
        return output
    ...
    def train_step(self, src_seq, src_pos, tgt_seq, optimizer, criterion, epoch_i):
        ...
        # log("Training prediction mask value: " + str(epoch_i * model.lambda_v))
